...
2. 匹配规则：
   - 从第一步诊断结果中提取：任务场景（对应一级场景标签）+ 用户输入中的关键概念
   - 对每个类型的知识库文件遍历所有条目
...
3. 结果处理：
   - 有命中条目 → 在对应 Agent prompt 中插入「参考知识库」字段
